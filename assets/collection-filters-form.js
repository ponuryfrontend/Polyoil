class CollectionFiltersForm extends HTMLElement {
  constructor() {
    ...
    this.filterBtnDesktop = this.querySelector('#filter-button-desktop');
    this.filtersDesktop = this.querySelector('#filters-desktop');

    this.debouncedOnSubmit = window.debounce((event) => {
      this.onSubmitHandler(event);
    }, 500);

    if(this.filterBtnDesktop) {
      this.filterBtnDesktop.addEventListener('click', e => {
        this.filtersDesktop.classList.toggle('is-active');
      })
    }
